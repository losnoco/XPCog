...
| `signalsmith-stretch` | **Went to `vendor/` instead.** This row planned an overlay port, but the stretch header and the signalsmith-linear FFT under it have to agree with each other version for version, upstream's tags trail what Cog ships against, and both are header-only -- so `vendor/signalsmith` takes them straight from Cog's ThirdParty, which keeps them the pair the stretcher was tuned on. | M4 |
| `rubberband` | **In vcpkg, overlaid for one patch.** The manifest's pinned baseline resolves 4.0.0#1, whose `mathmisc.h` uses bare `size_t` and does not include `<stddef.h>` -- AppleClang 21 stopped leaking the type and macOS CI failed inside the port. Upstream vcpkg fixed exactly this at 4.0.0#2 (`fix-size-t.patch`); this overlay is that port copied verbatim, and it retires with the next baseline bump. | M4 |
