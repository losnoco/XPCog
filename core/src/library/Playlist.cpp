...
std::optional<TrackId> Playlist::peekNext(TrackId from) {
    ...
    // Repeat-one ignored, as in next(): this answers for the button, and the
    // button that lands on the track already playing is a button that does
    // nothing.
    return nextEntry(from, /*ignoreRepeatOne=*/true);
}
