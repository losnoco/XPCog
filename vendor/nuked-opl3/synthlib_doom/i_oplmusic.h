...
class DoomOPL : public midisynth {
	private:
	// XPCog local change: was `fm_chip *opl;`, left indeterminate until
	// midi_init() runs -- and ~DoomOPL deletes it regardless. Anything that
	// constructs the synth only to read midi_bank_count() therefore frees a
	// garbage pointer, which is exactly what Cog's own
	// MSPlayer::enum_synthesizers does. It survives on macOS because a fresh
	// page reads as zero and delete nullptr is a no-op; MSVC's debug heap
	// fills new memory with 0xCD and the free faults immediately.
	fm_chip *opl = nullptr;
