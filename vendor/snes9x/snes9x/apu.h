...
namespace SNES {
/* XPCog local change: these were `struct`, while snes.hpp, smp.hpp and sdsp.hpp
   all define them as `class`. Clang warns (-Wmismatched-tags) and says why it
   matters: MSVC encodes `class` as V and `struct` as U in decorated names, so a
   translation unit that saw only this declaration mangles any signature
   mentioning these types differently from one that saw the definition, and the
   link fails. Cog builds snes9x for macOS alone, where it is merely a warning
   nobody sees. */
class CPU;
class SMP;
class DSP;
