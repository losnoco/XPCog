...
SettingEffect effectOf(std::string_view key) {
    ...
    // The server re-reads every one of them when it restarts, which is what the
    // branch in MainFrame::onSettingChanged does -- so a port change takes effect
    // without a relaunch.
