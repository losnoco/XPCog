...
///
/// Encoding: valid UTF-8 is taken as-is, anything else is treated as Latin-1.
/// Cog additionally tries GB18030 and CP1251 before falling back, which needs a
/// full conversion library; Latin-1 never fails, so no line is ever lost -- a
/// mis-decoded non-UTF-8 path simply will not resolve, which is the same outcome
/// Cog reaches when its guesses are wrong too.
